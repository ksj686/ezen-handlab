...
// public에 index가 있으면 index를 랜더링해버림
...
io.sockets.on("connection", (socket) => {
  ...
  console.log(
    "connection 이벤트 핸들러 >> 클라이언트가 접속했어요 socket.id: " + uid
  );
  //   클이 메시지를 보낼때 'echo' 이벤트를 발생시켜 메시지를 보냄 => 서버쪽에서 수신
  socket.on("echo", (data) => {
    console.log("from client >> " + data);
    // io.sockets.emit("sendAll", data); // 접속한 모든 클에게 메시지 전송 => 클에서 sendAll 이벤트를 수신(on('sendAll'))
    // socket.broadcast.emit("sendBroad", data);
    io.sockets.to(uid).emit("sendOne", data); // 방금 접속한 사용자에게만 메시지를 보냄
  });
});
