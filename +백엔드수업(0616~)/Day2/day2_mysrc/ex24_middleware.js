...
app.get("/", (req, res) => {
  ...
  if (req.user == "Hong") throw new Error("Hond은 절대로 안돼!!");
  res.end();
});
...
function auth(req, res, next) {
  ...
  if (!req.headers || !req.headers.authorization) {
    return res
      .status(401)
      .end(`<h1>${req.user}는 인증받지 않은 사용자입니다 - 접근 불가!</h1>`); // 인증받지 않은 사용자
  }
  next();
}
...
app.get("/mypage", auth, (req, res) => {
  ...
  res.write(`<h2>회원 인증을 거쳐야 들어오는 페이지 입니다</h2>`);
  res.write(`<h2>${req.user}님 환영합니다</h2>`);
  res.end();
});
